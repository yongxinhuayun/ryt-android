 ...
 * 
 */
public class ZuoPinYearValidation extends ValidationExecutor {

	@Override
	public boolean doValidate(Context context, String text) {
		if(text.isEmpty()){
			ToastUtil.showShort(AppApplication.getSingleContext(), "年份不能为空！");
			return false;
		}
		String regex = "^(19|20)\\d{2}$";
		boolean result = Pattern.compile(regex).matcher(text).find();
		if (!result) {
			ToastUtil.showShort(AppApplication.getSingleContext(), "年份格式不正确！");
			return false;
		}
		return true;
