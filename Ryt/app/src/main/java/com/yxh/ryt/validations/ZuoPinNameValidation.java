 ...
 * 
 */
public class ZuoPinNameValidation extends ValidationExecutor {

	@Override
	public boolean doValidate(Context context, String text) {
		if(text.isEmpty()){
			ToastUtil.showShort(AppApplication.getSingleContext(), "作品名称不能为空！");
			return false;
		}
		String regex = "^\\w{0,20}$";
		boolean result = Pattern.compile(regex).matcher(text).find();
		if (!result) {
			ToastUtil.showShort(AppApplication.getSingleContext(), "作品名称最多输入20个字符！");
			return false;
		}
		return true;
